...
const create_comment = (req, res) => {
    if(req.user) {
        ...
        try {
            if(req.body.text == undefined || req.body.postId == undefined) {
                throw "Fill all the required fields"
            } 
            if(valid.test(req.body.text) == false)  {
                throw "Text can't be empy"
            }             
            new Comment({user: {id: req.user._id, name: req.user.name, avatar : req.user.avatar ? req.user.avatar : undefined}, text: req.body.text, postId: req.body.postId}).save()
